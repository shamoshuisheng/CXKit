...
@implementation CXLocalized
+ (CXLocalized *)sharedInstance{
    static CXLocalized *language=nil;
    static dispatch_once_t onceToken;
    dispatch_once(&onceToken, ^{
        language = [[CXLocalized alloc] init];
    });
    return language;
}
...
- (void)systemLanguage{
    ...
    if([languageCode hasPrefix:@"zh-Hant"]){
        ...
        languageCode = @"ja";//日语
    }else if([languageCode hasPrefix:@"ms-MY"]){
        languageCode = @"ms-MY";//马来西亚
