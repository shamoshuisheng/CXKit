...
NSString* const CXStickHeaderWaterCellKind = @"WaterfallCell";
NSString* const CXStickHeaderWaterDecorationKind = @"Decoration";
...
- (void)setup {
    ...
    self.headAlignment = CXStickHeaderAlignmentLeft;
...
- (UICollectionViewLayoutAttributes *)layoutAttributesForItemAtIndexPath:(NSIndexPath *)indexPath {
    ...
    return self.layoutInfo[CXStickHeaderWaterCellKind][indexPath];
...
//    HXLog(@"sectionsHeights---%@",self.sectionsHeights);
    if (self.delegate && [self.delegate respondsToSelector:@selector(collectionView:andHeight:)]) {
        [self.delegate collectionView:self.collectionView andHeight:[self.sectionsHeights.firstObject floatValue]];
    }
...
- (void) calculateItemsAttributes {
    ...
    newLayoutInfo[CXStickHeaderWaterCellKind] = cellLayoutInfo;
...
- (CGRect)frameForWaterfallCellIndexPath:(NSIndexPath *)indexPath {
    ...
    for (NSInteger column = 0; column < [self.columnsCountArray[indexPath.section] integerValue]; column++) {
        #pragma mark ===首先的行间距===
        if (self.topLineSpace > 0) {
            columnsHeights[column] = [self headerHeightForIndexPath:indexPath] + self.topLineSpace;
        }else{
    ...
    for (NSInteger item = 0; item < indexPath.item; item++) {
        ...
        if (self.lineSpace > 0) {
            columnsHeights[currentColumn] += self.lineSpace;
        }else{
